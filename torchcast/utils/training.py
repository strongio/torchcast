...
from typing import Generator, Union, Type, Sequence, Tuple
...
class BaseTrainer:
    """
    Base class; child classes need to implement `_get_closure`
    """
    ...
    def __init__(self,
        ...
        self.module = module
        self.optimizer = optimizer if isinstance(optimizer, Optimizer) else optimizer(module.parameters())
        self._device = None
...
class StateSpaceTrainer(BaseTrainer):
    ...
    def _get_closure(self, batch: TimeSeriesDataset) -> callable:
        y = batch.tensors[0]
        kwargs = {}
        for i, (k, t) in enumerate(zip_longest(self.forward_kwargs, batch.tensors[1:])):
            if k is None:
                raise RuntimeError(
                    f"Found element-{i + 1} of the dataset.tensors, but `forward_kwargs` doesn't have enough "
                    f"elements: {self.forward_kwargs}"
                )
            if t is None:
                raise RuntimeError(
                    f"Found element-{i} of `forward_kwargs`, but `dataset.tensors` doesn't have enough "
                    f"elements: {batch}"
                )
            kwargs[k] = t
...
class SeasonalEmbeddingsTrainer(BaseTrainer):
    ...
    def times_to_model_mat(self, times: np.ndarray) -> torch.Tensor:
        out = []
        if self.yearly:
            out.append(torch.as_tensor(fourier_model_mat(times, K=self.yearly, period='yearly')))
        if self.weekly:
            out.append(torch.as_tensor(fourier_model_mat(times, K=self.weekly, period='weekly')))
        if self.daily:
            out.append(torch.as_tensor(fourier_model_mat(times, K=self.daily, period='daily')))
        for period, k in self.other:
            out.append(torch.as_tensor(fourier_model_mat(times, K=k, period=period)))
